...
__global__ void copy_array(float *u, float *u_prev, int N) {
  int i = threadIdx.x;
  int j = threadIdx.y;
  int I = blockIdx.y * BLOCKSIZE * N + blockIdx.x * BLOCKSIZE + j * N + i;
  if (I >= N * N) {
    return;
  }
  u_prev[I] = u[I];
...
__global__ void update(float *u, float *u_prev, int N, float h, float dt,
  ...
  __shared__ float u_prev_sh[BLOCKSIZE][BLOCKSIZE];
  ...
  bool bound_check =
      ((I > N) && (I < N * N - 1 - N) && (I % N != 0) && (I % N != N - 1));
  bool block_check = ((i > 0) && (i < BLOCKSIZE - 1) && (j > 0) && (j < BLOCKSIZE - 1));

  // if not on block boundary do
  if (block_check) {
    u[I] = u_prev_sh[i][j] + alpha * dt / h / h *
                                 (u_prev_sh[i + 1][j] + u_prev_sh[i - 1][j] +
                                  u_prev_sh[i][j + 1] + u_prev_sh[i][j - 1] -
                                  4 * u_prev_sh[i][j]);
  }
  // if not on boundary
  else if (bound_check)
