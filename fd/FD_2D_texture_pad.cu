...
// GPU kernels
__global__ void copy_kernel(float *u, float *u_prev, int N, int BSZ,
                            int N_max) {
  // Setting up indices
  int i = threadIdx.x;
  int j = threadIdx.y;
...
__global__ void update(float *u, float *u_prev, int N, float h, float dt,
                       float alpha, int BSZ, int N_max) {
  // Setting up indices
  int i = threadIdx.x;
  int j = threadIdx.y;
  int x = i + blockIdx.x * BSZ;
...
int main(int argc, char **argv) {
  ...
  cudaBindTexture2D(NULL, tex_u, u_d, desc, N_max, N_max,
                    sizeof(float) * N_max);
  cudaBindTexture2D(NULL, tex_u_prev, u_prev_d, desc, N_max, N_max,
                    sizeof(float) * N_max);
  ...
  // Loop
  dim3 dimGrid(int((N_max - 0.5) / BLOCKSIZE) + 1,
               int((N_max - 0.5) / BLOCKSIZE) + 1);
  dim3 dimBlock(BLOCKSIZE, BLOCKSIZE);
  double start = get_time();
  for (int t = 0; t < steps;
       t++) { // The transfer of u to u_prev needs to be in separate kernel
    // as it's read only
